...
(function() {
  $(function() {
    $('#filter-container').nestedAttributes({
      bindAddTo: $('.add-filter')
    });
    return $('#images-container').nestedAttributes({
      bindAddTo: $('.add-photo')
    });
  });
  ...
  NestedAttributes = (function() {
    ...
    NestedAttributes.prototype.extractClone = function() {
      ...
      if (this.$restorableClone) {
        ...
        $record = this.options.$clone || this.$items.last();
...
(function() {
  ...
  nestedForm = {
    init: function() {
      return this.toggleForms();
    },
    toggleForms: function() {
      return $(document).on('click', '.show-nested', function() {
        if ($(this).text() === 'Показать') {
          $(this).text('Скрыть');
          return $(this).closest('.nested-form').find('.image-wrapper').show();
        } else {
          $(this).text('Показать');
          return $(this).closest('.nested-form').find('.image-wrapper').hide();
        }
      });
    }
  };
...
(function() {
  ...
  tabs = {
    ...
    cloneTab: function() {
      return $(document).on('click', '.add-tab', function() {
        var cloned, id, last;
        last = $('.tabs .controls.well').last();
        cloned = last.clone();
        cloned.find('input').val('');
        cloned.find(".cke").remove();
        $('.tabs').append(cloned);
        id = "tab-body-" + (last.index() + 1);
        cloned.find('textarea').val('').attr('id', id);
    ...
    removeTab: function() {
      return $(document).on('click', '.remove-tab', function() {
        if ($('.tabs .controls.well').length !== 1) {
          return this.closest('.controls.well').remove();
        }
      });
    }
  };
...
(function() {
  ...
  videos = {
    ...
    cloneVideo: function() {
      return $(document).on('click', '.add-video', function() {
        var cloned, last;
        last = $('.videos .controls.well').last();
        cloned = last.clone();
        cloned.find('input').val('');
        return $('.videos').append(cloned);
      });
    },
    removeVideo: function() {
      return $(document).on('click', '.remove-video', function() {
        if ($('.videos .controls.well').length !== 1) {
          return this.closest('.controls.well').remove();
        }
      });
    }
